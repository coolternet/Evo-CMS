$(document).ready(function(){
    ...
    function ajax_call(type, data, callback) {
        ...
        $.ajax({
            type: type,
            dataType: 'json',
            url: "/modules/Evo-TSM/direct-ajax.php",
            data: data,
            success: callback,
    ...
    $("button[name=mark_critical]").on('click', function(){
        var tid = $('button[name=mark_critical]').attr("data-id");
        ajax_post({
    ...
    $("button[name=mark_normal]").on('click', function(){
        var tid = $('button[name=mark_normal]').attr("data-id");
        ajax_post({
                action  : 'mark_normal',
                tid     : tid
            },function(data){
        ...
        $('form#admin_create_ticket').validate({ // initialize the plugin
            ...
            submitHandler: function (form) {
                ajax_post({
                    ...
                    if(data.success){
                        alert('COMPLETED');
                        $(location).attr("href", "/admin/?p=Evo-TSM/tickets&state=opened")
                    }else{
                        alert(data.error);
                    }
                });
