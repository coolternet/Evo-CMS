...
function create_new_ticket($sujet,$content){
    ...
    try {
        ...
        // Utiliser \DB::Insert() avec une approche simplifiée
        $insert_data = [
            'sid' => $user->id,
            'subject' => $sujet,
            'short_desc' => $content,
        ...
        $result = \DB::Insert('tss_ticket', $insert_data);
        
        // Récupérer l'ID du dernier ticket créé par cet utilisateur
        $lastid = \DB::QueryAll('SELECT id FROM {tss_ticket} WHERE sid = :sid ORDER BY id DESC LIMIT 1', [':sid' => $user->id]);
        $ticket_id = isset($lastid[0]['id']) ? $lastid[0]['id'] : false;
        
        // Retourner l'ID même s'il est 0 (premier ticket)
        return $ticket_id !== false ? $ticket_id : false;
    } catch (Exception $e) {
