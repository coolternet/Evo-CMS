...
class GameServerQuery
{
	...
	public static function queryDoom3($host, $port)
	{
		...
		while (self::getInteger8($reponse) != 32 && $reponse != '') {
			$info['players'][] = array(
				'ping' => self::getInteger16($reponse),
	...
	public static function queryMinecraft($host, $port, $parseColors = false)
	{
		...
		if (isset($json['hostname'])) {
			$json['hostname'] = static::parseMinecraftString(str_replace(['<', '>'], ['&lt;', '&gt;'], $json['hostname']));
		}
	...
	public static function parseMinecraftString($string)
	{
		$colors = [
			1 => '#4c00c8',
			2 => '#00d400',
