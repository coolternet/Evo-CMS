...
?><div class="dropdown" id="user-dropdown">
	<a href="<?=App::getURL('user');?>" class="btn btn-secondary" data-bs-toggle="dropdown"><span class="account"><?= __('userdropdown.account') ?> (</span><strong><?= App::getCurrentUser()->username ?></strong><span class="account">)</span>
	<?php if ($alerts) echo ' <i class="fa fa-bell"></i> '. $alerts ?></a>
	<button type="button" class="btn btn-secondary dropdown-toggle dropdown-toggle-split account" data-bs-toggle="dropdown">
	...
	<div id="userdropdown" class="dropdown-menu" role="menu" style="margin-top: -4px;">
		...
		echo ' <span class="badge bg-dark">'. ltrim($friend_requests, '0') . '</span>';
		echo "</a>";

		echo '<a class="dropdown-item" href="'.App::getURL('mail').'"><i class="fa fa-envelope"></i> '.__('userdropdown.mailbox');
		echo ' <span class="badge bg-dark">'. ltrim($new_messages, '0') . '</span>';
		echo '</a>';
		...
		if (has_permission('moderator')) {
			echo '<a class="dropdown-item" href="' . App::getAdminURL() . '"><i class="fa fa-cog"></i> '.__('userdropdown.admin').'</a>';
		}
		?>
